...
TreeSpec makeTreeSpec(
  ...
  if (obj["type"].is_null()) {
    ...
    if (value) {
      ...
      return TreeSpec(value, isUsed);
    } else {
      return TreeSpec(value, false);
    }
  }
  ...
  for (const auto& child : childrenSpec) {
    children.push_back(makeTreeSpec(child, values, start + offset));
    // NOLINTNEXTLINE(*-narrowing-conversions)
    offset += children.back().numLeaves();
  }

  return TreeSpec(name, context, std::move(children), nodeDefCache);
}
...
TreeSpec treeSpecLoads(
  ...
  auto result = makeTreeSpec(obj[1], values, 0);

  TORCH_CHECK_EQ(result.numLeaves(), values.size());
  return result;
}
...
c10::IValue treeUnflatten(
    std::vector<c10::IValue> leaves,
    const TreeSpec& spec) {
  RECORD_USER_SCOPE("nativert::treeUnflatten");
...
std::vector<c10::IValue> treeFlattenFromArgs(
    ...
    const TreeSpec& spec) {
  RECORD_USER_SCOPE("nativert::treeFlattenFromArgs");
...
void leafApplyFromArgs(
    ...
    const std::unordered_map<std::string, c10::IValue>& kwargs,
    const TreeSpec& spec) {
  RECORD_USER_SCOPE("nativert::leafApplyFromArgs");
  TORCH_CHECK(!spec.isLeaf());
  TORCH_CHECK_EQ(spec.children().size(), 2);
