...
#include <torch/nativert/detail/Pytree.h>
...
[
  ...
  const auto spec = treeSpecLoads(jsonSpec, valuePtrs);
  ...
  auto tree = treeUnflatten(flats, spec);
  EXPECT_TRUE(tree.isList());
  EXPECT_EQ(tree.toListRef().size(), 5);

  EXPECT_TRUE(tree.toListRef().at(0).isTuple());
  EXPECT_EQ(tree.toListRef().at(0).toTupleRef().elements()[0], c10::IValue(0));
  EXPECT_EQ(tree.toListRef().at(0).toTupleRef().elements()[1], c10::IValue(1));

  EXPECT_TRUE(tree.toListRef().at(1).isInt());
  EXPECT_EQ(tree.toListRef().at(1), c10::IValue(2));

  EXPECT_TRUE(tree.toListRef().at(2).isGenericDict());
  EXPECT_EQ(tree.toListRef().at(2).toGenericDict().at("4"), c10::IValue(7));
  EXPECT_EQ(tree.toListRef().at(2).toGenericDict().at("5"), c10::IValue(8));
  EXPECT_EQ(tree.toListRef().at(2).toGenericDict().at("6"), c10::IValue(9));

  EXPECT_TRUE(tree.toListRef().at(3).isList());
  EXPECT_EQ(tree.toListRef().at(3).toListRef().at(0), c10::IValue(10));

  EXPECT_TRUE(tree.toListRef().at(4).isGenericDict());
  EXPECT_EQ(tree.toListRef().at(4).toGenericDict().at("11"), c10::IValue(12));

  const auto flattened = treeFlatten(tree, spec);
...
TEST(PytreeTest, NoVersion) {
  ...
  EXPECT_THROW({ treeSpecLoads(jsonSpec, valuePtrs); }, std::exception);
}

TEST(PytreeTest, NoField) {
  auto jsonSpec = R"(
[
  1,
  {
...
[
  ...
  auto [graph, valuePtrs] = makeValues(3);
  auto spec = treeSpecLoads(jsonSpec, valuePtrs);
  ...
  ASSERT_DEATH({ treeUnflatten(flats, spec); }, "Check failed");
}

TEST(PytreeTest, TooManyContext) {
  auto jsonSpec = R"(
[
  1,
  {
...
TEST(PytreeTest, DoubleRegister) {
  EXPECT_THROW(
      { registerPytreeNode("builtins.dict", NodeDef{}); }, std::exception);
}

TEST(PytreeTest, NotEnoughUnflatten) {
...
TEST(PytreeTest, TooManyUnflatten) {
  // Original data: [(0, 1), 2, {"4": 7, "5": 8, "6": 9}]
  auto jsonSpec = R"(
[
  1,
...
[
  ...
  auto flats = treeFlatten(c10::IValue{list}, spec);
...
TEST(PytreeTest, LeafApplyFromArgs) {
...
return(%o1)
    ...
    const auto spec = treeSpecLoads(jsonSpec, userInputs);

    std::vector<int> visited;
    auto fn = [&](const c10::IValue& leaf, const Value* value) {
      visited.push_back(value->id());
    };
    leafApplyFromArgs(fn, args, {}, spec);
...
TEST(PytreeTest, UnmatchedFlattenType) {
  // Original data: [(0, 1), 2, {"4": 7, "5": 8, "6": 9}]
  auto jsonSpec = R"(
[
  1,
  ...
  dict.insert("6", c10::IValue(9));
  EXPECT_THROW(
      { treeFlatten(c10::IValue{std::move(dict)}, spec); }, std::exception);
}

TEST(PytreeTest, UnmatchedDictFlatten) {
  // Original data: [(0, 1), 2, {"4": 7, "5": 8, "6": 9}]
  auto jsonSpec = R"(
[
  1,
  ...
  list.push_back(std::move(dict));
  ASSERT_DEATH(
      { treeFlatten(c10::IValue{std::move(list)}, spec); }, "Check failed");
}

TEST(PytreeTest, DictFlattenTest) {
  auto jsonSpec = R"(
[
  1,
  {
  ...
  c10::List<c10::IValue> list(c10::AnyType::get());
  list.push_back(std::move(dict));
  treeFlatten(c10::IValue{std::move(list)}, spec);
}

TEST(PytreeTest, UnmatchedTupleFlatten) {
  // Original data: [(0, 1), 2, {"4": 7, "5": 8, "6": 9}]
  auto jsonSpec = R"(
[
  1,
